...
internal sealed class SetInventoryCountVarianceReasonCommandHandler(
    IApplicationDbContext context,
    IUserContext userContext,
    IScopeAuthorizationService scopeAuthorizationService)
    : ICommandHandler<SetInventoryCountVarianceReasonCommand>
{
    public async Task<Result> Handle(SetInventoryCountVarianceReasonCommand command, CancellationToken cancellationToken)
    {
        ...
        if (count.Status is not (InventoryCountStatus.InProgress or InventoryCountStatus.Completed))
        {
        ...
        Result result = line.SetVarianceReason(command.Reason);
        if (result.IsFailure)
        {
            return result;
        }
